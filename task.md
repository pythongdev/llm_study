...
| **T-32** | NON-CODE | Áp vế `**Đóng khi.**` xuống **mọi** mục 🔴 đang có trong `finding.md`: mỗi mục khai lệnh sẽ chứng minh mệnh đề hết đúng, kèm vế `đỏ khi` | **Nạp:** [.claude/rules/chat-luong-finding.md](.claude/rules/chat-luong-finding.md) §1 §3 *(sau T-31)* · [quality/00-guideline-chat-luong.md](quality/00-guideline-chat-luong.md) §5 · **Đã chốt:** chỉ **thêm** vế — cấm đổi mệnh đề hay lệnh tái hiện của mục cũ, đổi mệnh đề là đổi finding · **Bẫy:** ba khuôn biên nhận giả ở guideline §5; mục nào chưa nghĩ ra lệnh thì viết `**Chưa đóng được bằng lệnh vì:**` + lý do, đừng bịa một lệnh xanh sẵn cho đủ vế — vế bịa còn tệ hơn vế thiếu vì cổng (c) sẽ xanh | T-31 T-23 | [F-22](finding.md#f-22) | Ba lệnh: cổng **(c)** ra **rỗng** — **đỏ khi** còn một mục 🔴 chưa khai `Đóng khi.` (hiện đỏ với 14 mã) · cổng **(a)** vẫn ra **rỗng** — **đỏ khi** việc thêm vế làm hỏng bốn vế cũ · `git show <sha vừa commit> -- finding.md \| grep -c '^-[^-]'` ra `0` — **đỏ khi** phiên xoá nội dung cũ để lấy chỗ cho vế mới | 🤖 *(khuôn đã chốt ở T-31, việc còn lại là cơ khí)* · [bảng soi ↓](#owner-t-32) | ⬜ T3 · [soi ↓](#cl-t-32) | Luật mới chỉ áp cho finding tương lai, còn 18 mã đang mở vẫn đóng được bằng lệnh tự chọn sau khi đã sửa — đúng vùng mù [F-22](finding.md#f-22) mô tả, ở khâu mỗi mã chỉ đi qua **một lần** | `Lane NON-CODE · T-32 · grep -n 'T-32' task.md · chỉ chạm finding.md · xong khi cổng (c) rỗng thì DỪNG, không sửa mệnh đề của mục nào` |
| **T-33** | NON-CODE | Tạo `quality/06-rang-buoc-kien-truc.md` — nhà duy nhất của **năm ràng buộc kiến trúc ẩn + dấu hiệu phải xem lại** ở `prompt-fullstack.md` §6.8, kèm con trỏ tới nó ở [quality/05-checklist.md](quality/05-checklist.md) §4 · [CLAUDE.md](CLAUDE.md) §2 | **Nạp:** `project_preparation/prompt-fullstack.md` §6.8 *(dòng 281 — `grep -n '^\*\*6\.8' project_preparation/prompt-fullstack.md` ra được)* · [quality/05-checklist.md](quality/05-checklist.md) §4 §5 · [quality/00-guideline-chat-luong.md](quality/00-guideline-chat-luong.md) §3 §5 · **Đã chốt:** **hướng (a) — file mới, không nhét thêm mục vào checklist.** Ba lý do nặng dần: checklist §4 **tự khai không sở hữu** §6.8, nhét vào là phá đúng ranh giới chính nó viết ra · biên nhận checklist ghim `grep -c '^- \[ \]'` ra **đúng 7, không hơn không kém** ở **ba** chỗ (checklist §5, ô biên nhận [T-04](#sổ-task), bảng [cl-T-04](#cl-t-04) đã thử làm đỏ bằng `5` và `8`) ⇒ hướng (b) làm cả ba đỏ để đổi lấy một mục · checklist trả lời *task này XONG chưa* — hỏi **một lần**, gắn với một task; §6.8 trả lời *cái gì không được phá* — đứng **mãi**, không gắn task nào: hai câu hỏi thì [CLAUDE.md §2](CLAUDE.md) đòi hai nhà · **Bẫy:** [quality/05-checklist.md](quality/05-checklist.md) đang **75/75 dòng**, kịch trần §5 ⇒ con trỏ phải **nối vào dòng ranh giới §4 có sẵn**, thêm một dòng mới là vỡ trần ngay trong commit tạo ra nó · [CLAUDE.md](CLAUDE.md) §2 đang **18/14 dòng**, đã vượt trần mục §8 (đúng vùng [F-08](finding.md#f-08) mô tả: trần mục không có lệnh kiểm nên trôi trong im lặng) ⇒ thêm một hàng bảng phải **gộp** một hàng cũ trong cùng commit, nới trần là chọn đường dễ · dòng này là **task chứ không phải finding**: nó mở đầu bằng *"chưa có nhà cho §6.8"*, mà sổ lỗi đòi một mệnh đề **sai hết đúng** — ở đây không mệnh đề nào sai, chỉ có một chỗ trống ([CLAUDE.md §5](CLAUDE.md) · [.claude/rules/chat-luong-finding.md](.claude/rules/chat-luong-finding.md)); viết nó vào `finding.md` là trộn sổ và mã đó sẽ không bao giờ đóng được · chỉ **ghi ra** ràng buộc, **gắn** dấu hiệu vào một cổng cưỡng chế (đích `check` của `Makefile`, hay một vế checklist) là task riêng **chưa mở** — gộp vào đây là chạm 2 lane, vượt [§6](CLAUDE.md) | — | — | Bốn vế, chạy từ gốc repo: `test -e quality/06-rang-buoc-kien-truc.md` không kêu — **đỏ khi** chưa có nhà · `awk '/1 instance/{a=1}/hàng đợi/{b=1}/polling 20s/{c=1}/200 món/{d=1}/1 VPS/{e=1}END{print a+b+c+d+e}' quality/06-rang-buoc-kien-truc.md` in ra **đúng 5** (= 5 ràng buộc §6.8, **không** phải ngưỡng dưới) — **đỏ khi** in ra `< 5` ⇒ thiếu ràng buộc; vế này đếm **nội dung khớp nguồn**, nên thêm bao nhiêu dòng rác cũng không làm nó xanh · `grep -c '^- \[ \]' quality/05-checklist.md` vẫn ra **đúng 7** và `wc -l < quality/05-checklist.md` ra `<= 75` — **đỏ khi** ra `8` ⇒ phiên đã đi hướng (b) trái ô `Đã chốt`, hoặc ra `76` ⇒ con trỏ §4 được thêm thành dòng mới thay vì nối vào dòng có sẵn · `awk '/^## §2/{f=1;n=0} f&&/^## §3/{print n; exit} f{n++}' CLAUDE.md` in ra **18** kèm `grep -c 'rang-buoc-kien-truc' CLAUDE.md` ra `>= 1` — **đỏ khi** số thứ nhất ra `19` ⇒ thêm hàng mà không gộp, hoặc số thứ hai ra `0` ⇒ nhà mới không có mặt trên bản đồ §2, đúng cách §6.8 mất tích lần đầu | 👤 *(gộp một hàng con trỏ ở `CLAUDE.md` §2 — bỏ một con trỏ là bỏ vĩnh viễn; rule §1.2b: sửa luật ở §2–§8)* · [bảng soi ↓](#owner-t-33) | ⬜ T3 · [soi ↓](#cl-t-33) | Trạm bếp chậm một hôm, phiên sau thấy vậy liền dựng thêm **bản BE thứ hai** cho *nhẹ tải* — mà SSE giữ kết nối trong bộ nhớ từng process, nên từ đó trạm bếp **mất đơn ngẫu nhiên** đúng giờ đông khách, lỗi khó lần nhất dự án; bảy vế checklist vẫn xanh hết, không lệnh nào trong repo kêu một tiếng | `Lane NON-CODE · T-33 · grep -n 'T-33' task.md · chỉ chạm quality/06-rang-buoc-kien-truc.md, một cụm ở quality/05-checklist.md §4, đúng một hàng CLAUDE.md §2 · xong bốn vế biên nhận thì DỪNG — không gắn dấu hiệu vào đích check của Makefile, đó là task khác chưa mở` |
| ~~**T-34**~~ ✅ | NON-CODE | Tạo `quality/01-chat-luong-noi-dung.md` — nhà duy nhất của câu hỏi *"nội dung có ĐÚNG ĐỂ XÂY không"*: sáu trục N1–N6, thủ tục chấm bốn bước, ba nhãn tại chỗ; nối con trỏ bằng cách **gộp** một hàng [CLAUDE.md](CLAUDE.md) §2 và hai dòng header [quality/00-guideline-chat-luong.md](quality/00-guideline-chat-luong.md) | **Nạp:** [quality/00-guideline-chat-luong.md](quality/00-guideline-chat-luong.md) §2 §3 §4 §5 §7 · [finding.md](finding.md#f-33) · [project_preparation/prompt-fullstack.md](project_preparation/prompt-fullstack.md) §6.8 §7 · **Đã chốt:** owner nêu 2026-08-25 — guideline hiện chỉ đo *số lượng và hình dạng*, chưa có luật nào đo **nội dung**; ba tiêu chí owner gọi tên: hợp dự án, hợp pha hiện tại, đối chiếu best practice ngoài · **file mới chứ không nhét thêm vào guideline**: guideline trả lời *dùng lại được không*, file mới trả lời *có đúng thứ để xây không* — hai câu hỏi thì [CLAUDE.md §2](CLAUDE.md) đòi hai nhà; guideline cũng đang 107/115 dòng, không còn chỗ cho sáu trục · **Bẫy:** [CLAUDE.md](CLAUDE.md) §2 đang **18/14 dòng** và biên nhận của [T-33](#sổ-task) ghim con số 18 ⇒ **phải gộp** vào hàng `quality/00` có sẵn, thêm hàng mới là làm đỏ biên nhận task khác · năm trục §4 của guideline **cấm chép** sang file mới, chỉ được trỏ ⇒ trục nội dung phải là trục guideline **chưa có**, không phải bản diễn giải lại · số thứ tự tên file `quality/` đang va nhau — xem [F-40](finding.md#f-40), phiên này không sửa hệ tên | — | — | Sáu vế, chạy từ gốc repo: `test -e quality/01-chat-luong-noi-dung.md` không kêu — **đỏ khi** chưa có nhà · `grep -c '^\*\*N[1-6] · ' quality/01-chat-luong-noi-dung.md` ra **đúng 6** — **đỏ khi** ≠ 6, tức thiếu trục hoặc đẻ trục thứ bảy · `grep -c '^· \*Đỏ khi:\*' quality/01-chat-luong-noi-dung.md` ra **đúng 6** — **đỏ khi** < 6 ⇒ có trục không khai vế âm, đúng thứ [guideline §5](quality/00-guideline-chat-luong.md) cấm · `grep -c '01-chat-luong-noi-dung' CLAUDE.md` ra `1` **và** `grep -c '01-chat-luong-noi-dung' quality/00-guideline-chat-luong.md` ra `>= 1` — **đỏ khi** ra `0` ⇒ nhà mới không có trên bản đồ §2 · `awk '/^## §2/{f=1;n=0} f&&/^## §3/{print n; exit} f{n++}' CLAUDE.md` vẫn ra **18** — **đỏ khi** ra `19` ⇒ thêm hàng thay vì gộp, làm đỏ biên nhận [T-33](#sổ-task) · `wc -l < quality/01-chat-luong-noi-dung.md` ra `<= 125` và `awk 'length > 400' quality/01-chat-luong-noi-dung.md` rỗng | 👤 *(thêm một câu hỏi chất lượng thứ ba, áp cho mọi lane mọi pha — rule §1.2b: sửa luật ở `CLAUDE.md` §2)* · [bảng soi ↓](#owner-t-34) | ✅ T2 · [soi ↓](#cl-t-34) | Máy viết ra tài liệu sạch, nhất quán, dùng được — và **sai thứ**: pha 0 gọi tên bảng dữ liệu, thiết kế cho quán 50 chi nhánh, số liệu tự điền trông y hệt số owner chốt. Mọi cổng hiện có đều xanh, nên cái sai chỉ lộ ra lúc owner đọc tận mắt, và tới lúc đó đã có ba phiên xây tiếp lên trên nó | `Lane NON-CODE · T-34 · đã xong ở phiên 2026-08-25 · không mở lại — cưỡng chế sáu probe bằng hook là [T-35](#sổ-task)` |
| **T-35** | NON-CODE | Cưỡng chế sáu probe N1–N6 bằng hook `Stop` trong `.claude/settings.json`: phiên có sửa file mà câu trả lời không có phiếu sáu dòng thì hook **nhắc** | **Nạp:** [quality/01-chat-luong-noi-dung.md](quality/01-chat-luong-noi-dung.md) §5 §7 · [CLAUDE.md §8](CLAUDE.md) · **Đã chốt:** luật cần **chắc chắn** xảy ra thì nhà của nó là hook, câu chữ chỉ là lời đề nghị ([CLAUDE.md §8](CLAUDE.md)) · **Bẫy:** hook chặn bằng exit 2 mỗi lần kết phiên là cách nhanh nhất làm owner tắt hook — bản đầu chỉ **nhắc**, đừng chặn; và `.claude/settings.json` dùng chung với [T-15](#sổ-task) ⇒ làm sau T-15, đừng dựng hai hook `Stop` rời nhau | T-15 · T-36 | — | `python3 -c 'import json;json.load(open(".claude/settings.json"))'` không kêu; chạy tay lệnh trong hook trên một câu trả lời **không có** dòng `N1 ·` thì in ra nhắc, trên câu trả lời **có đủ sáu dòng** `N1 ·`…`N6 ·` thì im — **đỏ khi** cả hai lượt đều im ⇒ hook không đọc gì, hoặc cả hai lượt đều nhắc ⇒ hook nhắc vô điều kiện và sẽ bị tắt sau hai phiên | 👤 *(hook chạy ở **mọi** phiên, hỏng là hỏng toàn cục)* · [bảng soi ↓](#owner-t-35) | ⬜ T2 · [soi ↓](#cl-t-35) | Sáu trục sống bằng thiện chí: phiên nào bỏ qua §5 thì owner mất đúng phiên đó, và không dấu hiệu nào báo là đã mất — đúng chế độ hỏng mà chính `quality/01` dựng lên để chặn | `Lane NON-CODE · T-35 · grep -n 'T-35' task.md · chỉ chạm .claude/settings.json · hook chỉ NHẮC không CHẶN · xong thì DỪNG` |
| **T-36** | NON-CODE | Tạo `.claude/rules/chat-luong-noi-dung.md` — rule **mỏng** có `paths:` phủ nơi đầu ra nội dung ra đời, thân **chỉ trỏ** sang [quality/01-chat-luong-noi-dung.md](quality/01-chat-luong-noi-dung.md) §3 §4 §5; và thêm **vế thứ 6** vào [.claude/rules/quan-ly-du-an.md](.claude/rules/quan-ly-du-an.md) mục 6: mở lane ⇒ nới `paths:` rule đó | **Nạp:** [.claude/rules/chat-luong-finding.md](.claude/rules/chat-luong-finding.md) §1 §4 — mô hình rule mỏng chỉ-trỏ đang chạy thật · [.claude/rules/quan-ly-du-an.md](.claude/rules/quan-ly-du-an.md) mục 4 mục 6 · [finding.md](finding.md#f-43) · **Đã chốt:** owner chốt 2026-08-25 thứ tự A→B→C — nước A (hàng nhịp ở mục 4) đã xong cùng phiên, đóng [F-42](finding.md#f-42) · nhà của sáu trục vẫn là `quality/01`, rule mới **chỉ trỏ** · **Bẫy:** `paths: ["**"]` nạp ở mọi phiên là thuế mọi phiên ([CLAUDE.md §8](CLAUDE.md)) và luật phiên nào cũng thấy là luật phiên nào cũng bỏ qua ⇒ liệt kê mẫu · chép §3 §4 sang rule mới là đẻ nhà thứ hai, đúng gốc [F-04](finding.md#f-04) | — | [F-43](finding.md#f-43) | Ba lệnh, chạy từ gốc repo: `grep -c '01-chat-luong-noi-dung' .claude/rules/chat-luong-noi-dung.md` ra `>= 1` — **đỏ khi** ra `0`, tức rule mới không trỏ về nhà nào · `grep -cE '^..N[1-6] · ' .claude/rules/chat-luong-noi-dung.md` ra **`0`** — **đỏ khi** ra `>= 1`, tức rule chép trục thay vì trỏ · `grep -c '^6\. \*\*' .claude/rules/quan-ly-du-an.md` ra `1` — **đỏ khi** ra `0`; hôm nay đúng là `0` vì mục 6 mới có 5 vế, nên vế này **không** xanh sẵn | 🤖 *(rule mới, không đụng nhà thật nào)* · [bảng soi ↓](#owner-t-36) | ⬜ T2 · [soi ↓](#cl-t-36) | Bốn lane code mở ra mà nhịp chất lượng không theo: phiên viết code không thấy sáu probe, và code kiểu *sạch nhưng sai thứ* chỉ lộ ở phiên thứ ba, dưới dạng làm lại từ đầu | `Lane NON-CODE · T-36 · grep -n 'T-36' task.md · chỉ chạm .claude/rules/chat-luong-noi-dung.md + .claude/rules/quan-ly-du-an.md mục 6 · rule mới CHỈ TRỎ không chép · xong thì DỪNG` |
| ~~**T-37**~~ ✅ | BA | Gỡ dấu ⚠️ mồ côi ở [design/BA/04-yeu-cau.md](design/BA/04-yeu-cau.md) dòng 22 — ô Đ4 còn khai `nhà thật ⚠️ project_preparation/00-scope.md §4.4 (T-05 tạo)` trong khi T-05 **đã tạo** nhà đó; ⚠️ ở đây nghĩa *chưa có*, nay sai | **Nạp:** [.claude/rules/lane-ba.md](.claude/rules/lane-ba.md) · [design/BA/04-yeu-cau.md](design/BA/04-yeu-cau.md) §1 §2 · [project_preparation/00-scope.md](project_preparation/00-scope.md) §4.4 · **Đã chốt:** vế (b) của [F-31](finding.md#f-31); vế (a) đã xử 2026-08-25 ở `prompt-fullstack.md` §9.3 · lane NON-CODE **không** sở hữu `design/BA/**` ([CLAUDE.md §1](CLAUDE.md)) nên vế này phải đợi phiên BA | — | [F-31](finding.md#f-31) *(vế b — đóng nốt)* | Ba vế, chạy từ gốc repo: `grep -n '00-scope' design/BA/04-yeu-cau.md \| grep -c '⚠️'` ra **`0`** — **đỏ khi** ra `>= 1`, tức ⚠️ mồ côi còn đó · `grep -c '00-scope' design/BA/04-yeu-cau.md` ra **`>= 1`** — **đỏ khi** ra `0`, tức phiên xoá luôn con trỏ thay vì gỡ mỗi dấu ⚠️ · bốn cổng lane BA ở [design/BA/04-yeu-cau.md §2](design/BA/04-yeu-cau.md) vẫn xanh, riêng `grep -c '^| Đ[0-9]' design/BA/04-yeu-cau.md` ra **`4`** — **đỏ khi** ra `3`, tức sửa ô Đ4 làm hỏng hàng bảng | 🤖 *(gỡ một dấu đã sai, không đổi mệnh đề nào)* · [bảng soi ↓](#owner-t-37) | ✅ T2 · [soi ↓](#cl-t-37) | Lane BA đọc ô Đ4, tin rằng §4.4 **chưa có nhà**, rồi tự chốt lại thành phần suất bán lần thứ hai — mà §4.4 là số **owner chốt 2026-08-19**; từ đó bếp và giá đi theo hai bản khác nhau, và cái sai chỉ lộ lúc owner đọc phiếu bếp | `Lane BA · T-37 · grep -n 'T-37' task.md · chỉ chạm design/BA/04-yeu-cau.md dòng 22 · gỡ ⚠️ giữ con trỏ · xong ba vế thì DỪNG, không viết nội dung Đ1 Đ2 Đ3` |
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Phạm vi bán + bảng giá món; chỗ tài liệu chưa chốt ghi `GIẢ ĐỊNH:` kèm mức rủi ro | `git show c1a789e -- project_preparation/00-scope.md` | `project_preparation/00-scope.md` **(mới)** › cả file |
| Dòng *Phạm vi & giá món* hết ⚠️, trỏ vào nhà mới | `git show c1a789e -- CLAUDE.md` | [CLAUDE.md](CLAUDE.md) §2 › **đúng một dòng** bảng |
| §3.2 rút còn 4 dòng trỏ lên `00-scope.md`; §9.3 khai 10 ca giá là **dẫn xuất** từ §4.2 | `git show c1a789e -- project_preparation/prompt-fullstack.md` | [prompt-fullstack.md](project_preparation/prompt-fullstack.md) §3.2 + §9.3 › hai chỗ |
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Rule riêng của lane deploy | — | `.claude/rules/lane-devops.md` **(mới)** › frontmatter `paths:` trỏ `Makefile`, `deploy/**`, `.env*` |
| Dòng DEVOPS hết ⚠️ | — | [CLAUDE.md](CLAUDE.md) §1 › **đúng một dòng** DEVOPS |
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Rule riêng của lane dữ liệu | — | `.claude/rules/lane-db.md` **(mới)** › frontmatter `paths:` |
| Nhà của thiết kế dữ liệu | — | `design/data_base/` **(mới)** |
| Dòng DB hết ⚠️ | — | [CLAUDE.md](CLAUDE.md) §1 › **đúng một dòng** DB |
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Rule riêng của lane backend | — | `.claude/rules/lane-be.md` **(mới)** › frontmatter `paths:` |
| Nhà của thiết kế backend | — | `design/backend/` **(mới)** |
| Dòng BE hết ⚠️ | — | [CLAUDE.md](CLAUDE.md) §1 › **đúng một dòng** BE |
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Rule riêng của lane frontend | — | `.claude/rules/lane-fe.md` **(mới)** › frontmatter `paths:` |
| Nhà của thiết kế frontend | — | `design/frontend/` **(mới)** |
| Dòng FE hết ⚠️ | — | [CLAUDE.md](CLAUDE.md) §1 › **đúng một dòng** FE |
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Mỗi chỗ trỏ tới file chưa có được đánh ⚠️ + mã task tạo ra nó, **ngay tại chỗ trỏ** | — | [project_preparation/prompt-fullstack.md](project_preparation/prompt-fullstack.md) › đúng 8 chỗ [F-02](finding.md#f-02) liệt kê |
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| `paths:` nhận thêm `CLAUDE.md` và `.claude/rules/**`, để phiên mở lane được nạp quy trình §6 | `git show b520e7c -- .claude/rules/quan-ly-du-an.md` | [.claude/rules/quan-ly-du-an.md](.claude/rules/quan-ly-du-an.md) › frontmatter `paths:` › thêm **đúng 2 dòng** |
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Rule giữ *viết thế nào*; §7 và Phần IV gộp về một bản | — | [.claude/rules/quan-ly-du-an.md](.claude/rules/quan-ly-du-an.md) › §1.1 + §7 |
| Guideline giữ *vì sao có ô này* | — | [project_preparation/huong-dan-viet-task-md.md](project_preparation/huong-dan-viet-task-md.md) › Phần IV + Phần V |
| Nói ra bên nào giữ vế nào, nếu không lần sau lại mọc bản thứ hai | — | [CLAUDE.md](CLAUDE.md) §2 › **đúng 2 dòng** bảng |

**Đạt khi:** `git show --stat <sha vừa commit>` ra **đúng 3 file**, và `CLAUDE.md` chỉ đổi hai dòng bảng §2 — **không** thêm mục mới (trần §8).
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Thoát dấu ngăn cột trong ô biên nhận, để dòng đủ 10 cột trở lại | — | [task.md](task.md) › **đúng dòng T-05**, ô `Đầu ra kiểm chứng được` |
| Thêm phép đếm cột + siết mẫu dò finding thành hai chữ số có biên | — | [.claude/rules/quan-ly-du-an.md](.claude/rules/quan-ly-du-an.md) › §5.2 |
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Khuôn bảng thay đổi 5 cột, khuôn bảng soi của owner, lệnh tự rà — `paths: **` nên nạp ở mọi phiên | `git show b520e7c -- .claude/rules/bao-cao-thay-doi.md` | [.claude/rules/bao-cao-thay-doi.md](.claude/rules/bao-cao-thay-doi.md) **(mới)** › §1 §2 §3 §4 |
| Mỗi task có một bảng soi ba cột thay cho ô chữ chạy dài | `git show ac02f22 -- task.md` | [task.md](task.md) › §Owner kiểm tra **(mục mới)** + 16 ô `Owner kiểm tra` rút gọn |
| Ô `Owner kiểm tra` đổi đặc tả; thêm một nhịp kiểm tra và một lệnh tự rà | `git show ac02f22 -- .claude/rules/quan-ly-du-an.md` | [.claude/rules/quan-ly-du-an.md](.claude/rules/quan-ly-du-an.md) › §1.1 + §1.2 + §4 + §5.2 |
| Vế đánh ✅ từ **ba** lên **bốn**: thiếu bảng thay đổi ⇒ phiên chưa xong | `git show b520e7c -- CLAUDE.md` | [CLAUDE.md](CLAUDE.md) › §2 **một dòng** bảng, §4 › vế **(4)** |
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Hook `Stop` **nhắc** phiên dán bảng thay đổi khi cây còn bẩn | — | `.claude/settings.json` **(mới)** › khoá `hooks.Stop` |
| Ghi ra ai sở hữu `.claude/settings.json` — hiện §1 chưa có chủ | — | [CLAUDE.md](CLAUDE.md) §1 › **đúng một dòng** NON-CODE, cột *Lane sở hữu file nào* |
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Hai lệnh rà tính duy nhất của mã finding: `uniq -d` trên `^### F-`, và đối chiếu số mục với số dòng bảng | — | [.claude/rules/quan-ly-du-an.md](.claude/rules/quan-ly-du-an.md) › §5.2 |
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Nhà chất lượng của pha 0: mệnh đề riêng + lệnh đo riêng + ranh giới pha | — | `quality/01-pha-0-ba.md` **(mới)** › cả file |
| Đoạn liệt kê 6 pha hết ⚠️ ở đúng vế pha 0 | — | [quality/00-guideline-chat-luong.md](quality/00-guideline-chat-luong.md) §7 › vế `0 · BA` |
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Ô `Đầu ra kiểm chứng được` bắt buộc khai trạng thái hỏng làm nó đỏ | — | [.claude/rules/quan-ly-du-an.md](.claude/rules/quan-ly-du-an.md) §1.1 › hàng `Đầu ra kiểm chứng được` |
| Vế 2 của định nghĩa XONG trỏ sang thang T0–T3 thay vì dừng ở "hình dạng" | — | [.claude/rules/quan-ly-du-an.md](.claude/rules/quan-ly-du-an.md) §3 › vế 2 |

**Đạt khi:** đúng **1** file trong commit; rule dài thêm **≤ 6 dòng** (`git show --stat <sha vừa commit>` cho `+6` trở xuống) — luật mới phải trỏ, không chép thang.
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Mỗi ô `Đầu ra kiểm chứng được` có thêm vế âm, lệnh cũ giữ nguyên | — | [task.md](task.md) §Sổ task › **mọi** dòng `T-xx` |

**Đạt khi:** diff **chỉ thêm**, không xoá lệnh nào đang có (`git show <sha vừa commit> -- task.md | grep "^-" | grep -c "grep\|test -e\|wc -l"` ra `0`); và số dòng bảng không đổi.
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Khuôn một mục `### F-xx` (4 vế mở / 6 vế đóng), 4 phép thử một mệnh đề, 2 cổng lệnh, 3 trường hợp **không** mở finding | `git show e3bb4ae -- .claude/rules/chat-luong-finding.md` | `.claude/rules/chat-luong-finding.md` **(mới)** › §1–§5 |
| Hàng *"Cái gì đang sai"* của §2 trỏ thêm nhà của **khuôn**, không thêm hàng mới | `git show e3bb4ae -- CLAUDE.md` | [CLAUDE.md](CLAUDE.md) §2 › **đúng một hàng**, hàng thứ hai của bảng |
| Hai dòng sổ: T-22 (việc này) và T-23 (vá 5 mục cho cổng (a) xanh) | `git show e3bb4ae -- task.md` | [task.md](task.md) §Sổ task › hai dòng ngay dưới `T-21` · §Owner kiểm tra › hai bảng soi cuối file |

**Đạt khi:** đúng **3** file trong commit (`git show --stat e3bb4ae` ra 3 dòng) — `finding.md` **không** có mặt, nó là đối tượng của luật chứ không phải chỗ chứa luật; `CLAUDE.md` đổi đúng **1** dòng và `wc -l < CLAUDE.md` không tăng; và rule mới **trỏ chứ không chép** phần mở đầu `finding.md` — `grep -c 'Rút không ra luật' .claude/rules/chat-luong-finding.md` ra `0` (câu đặc trưng của phần mở đầu đó; có mặt nghĩa là đã chép).
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| F-06 có `Lệnh tái hiện.`; F-13 có `Vì sao nó không tự mất đi.`; F-07, F-08, F-12 có `Chưa đề xuất được vì:` | — | [finding.md](finding.md) › mục `### F-06`, `### F-07`, `### F-08`, `### F-12`, `### F-13` |

**Đạt khi:** cổng (a) ra rỗng, và diff **chỉ thêm** — `git show <sha vừa commit> -- finding.md | grep -c '^-[^-]'` ra `0`. Có dòng `-` nghĩa là phiên đã sửa nội dung mệnh đề cũ, việc đó không nằm trong T-23.
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Cột thứ 9 của sổ task + 23 mục `### cl-T-xx` mới | `git show 3abe7e8 -- task.md` | [task.md](task.md) › §Sổ task (dòng tiêu đề + 23 dòng task) và §Chất lượng đầu ra **(mới)** |
| Đặc tả ô `Chất lượng`, chỉ số đếm ô từ phải, hai lệnh tự rà mới | `git show 3abe7e8 -- .claude/rules/quan-ly-du-an.md` | [.claude/rules/quan-ly-du-an.md](.claude/rules/quan-ly-du-an.md) › §1 khuôn · §1.1 · §1.2 · §4 · §5.2 |
| Tầng step trỏ thêm sang cột mới, không chép nội dung | `git show 3abe7e8 -- quality/00-guideline-chat-luong.md` | [quality/00-guideline-chat-luong.md](quality/00-guideline-chat-luong.md) › §7 gạch đầu dòng *Tầng step* |

**Đạt khi:** `git show --stat 3abe7e8` ra **đúng 3 file**. File thứ tư ⇒ phiên đã lấn phạm vi ([CLAUDE.md §6](CLAUDE.md)), đừng nhận.
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Bảng 4 kênh bán, mỗi kênh đủ ba ô *ai bấm · gắn bàn · tiền thu ở khâu nào* | — | `design/BA/01-kenh-ban.md` **(mới)** › §1 bảng kênh |
| Hàng `Đ1` ở bảng đầu ra bắt buộc hết ⚠️: đường dẫn thành thật | — | [design/BA/04-yeu-cau.md](design/BA/04-yeu-cau.md) §1 › **đúng ô `Nhà của nó` của hàng `Đ1`** |

**Đạt khi:** `git show --stat <sha vừa commit>` ra **đúng 2 file**, cả hai trong `design/BA/`. File thứ ba — nhất là file ngoài `design/BA/` — ⇒ lấn lane, đừng nhận.
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Thân lệnh (e): từ hai dòng chú thích + `<path>` thành vòng lặp đọc `task.md`, lọc theo dòng đã gạch | `git show fc8f475 -- .claude/rules/bao-cao-thay-doi.md` | [.claude/rules/bao-cao-thay-doi.md](.claude/rules/bao-cao-thay-doi.md) §3 › khối `# e.` |
| Một câu nói rõ (a)–(e) nào **bắt buộc** chạy trước khi commit | `git show fc8f475 -- .claude/rules/bao-cao-thay-doi.md` | [.claude/rules/bao-cao-thay-doi.md](.claude/rules/bao-cao-thay-doi.md) §3 › đoạn văn dưới khối lệnh |

**Đạt khi:** `git show --stat fc8f475` ra **đúng 1 file**. Đụng `task.md` ⇒ đã lấn sang T-27, đừng nhận.
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| 12 lệnh của ba task đã gạch: `HEAD` → sha ghim | — | [task.md](task.md) §Owner kiểm tra › `### owner-T-14` `### owner-T-22` `### owner-T-24` |
| Một vế thêm vào bước ghi sổ: commit xong thì thay `HEAD` bằng sha vừa tạo | — | [CLAUDE.md](CLAUDE.md) §4 › đoạn về bảng thay đổi |
| Ví dụ `owner-T-02` in trong rule đổi từ `HEAD` sang sha — chỗ phiên sau chép | — | [.claude/rules/bao-cao-thay-doi.md](.claude/rules/bao-cao-thay-doi.md) §2 › bảng ví dụ |

**Đạt khi:** `git show --stat <sha vừa commit>` ra **đúng 3 file**, và diff `task.md` **chỉ** chạm ba mục `owner-T-14` `owner-T-22` `owner-T-24`. Chạm mục thứ tư ⇒ đã đổi sha cho task chưa làm, trả lại.
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Mẫu dò dòng task: `^\| \*\*T-` → `^\| ~*\*\*T-`, chấp nhận dòng đã gạch | — | [.claude/rules/quan-ly-du-an.md](.claude/rules/quan-ly-du-an.md) §5.2b › khối `for L in ba db be fe devops` |
| Vế thứ sáu của quy trình mở lane: chỗ hụt ghi vào `finding.md`, không ghi vào file lane | — | [.claude/rules/quan-ly-du-an.md](.claude/rules/quan-ly-du-an.md) §6 › danh sách vế |

**Đạt khi:** `git show --stat <sha vừa commit>` ra **đúng 1 file**. Đụng `design/BA/` ⇒ đã sang lane BA, trả lại.
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Lệnh hàng `Đúng` của bảng chất lượng, và vế 3 ô biên nhận `T-02` | — | [task.md](task.md) › `### cl-T-02` hàng `Đúng`, và §Sổ task dòng `T-02` ô `Đầu ra kiểm chứng được` |
| Dòng ví dụ dùng khuôn `[^ )]*` — chỗ 22 bảng `cl-T-xx` còn lại sẽ chép | — | [quality/00-guideline-chat-luong.md](quality/00-guideline-chat-luong.md) §5 › ví dụ lệnh soi |

**Đạt khi:** `git show --stat <sha vừa commit>` ra **đúng 2 file**, và diff `task.md` không chạm bảng `cl-` nào ngoài `cl-T-02`.
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Khuôn message tách commit **làm** task khỏi commit **mở** task | — | [CLAUDE.md](CLAUDE.md) §4 › đoạn `Commit: stage bằng...` |
| Cổng *sổ lệch git* thêm vào khối lệnh tự rà | — | [.claude/rules/quan-ly-du-an.md](.claude/rules/quan-ly-du-an.md) §5.2 › cuối khối |

**Đạt khi:** `git show --stat <sha vừa commit>` ra **đúng 2 file**, và `git log --oneline -1` của chính commit này đã theo khuôn mới.
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Một hàng mới trong bảng khuôn: vế `**Đóng khi.**`, bắt buộc khi **mở** | — | [.claude/rules/chat-luong-finding.md](.claude/rules/chat-luong-finding.md) §1 › bảng khuôn, hàng sau `Cách sửa đề xuất.` |
| Nhánh cổng **(c)**: mục 🔴 không khai `Đóng khi.` thì in ra | — | [.claude/rules/chat-luong-finding.md](.claude/rules/chat-luong-finding.md) §3 › khối ```bash |
| Hai dòng có sẵn của §3 bị **gộp** để trả chỗ cho nhánh mới | — | [.claude/rules/chat-luong-finding.md](.claude/rules/chat-luong-finding.md) §3 › văn xuôi quanh khối lệnh |

**Đạt khi:** `git show --stat <sha vừa commit>` ra **đúng 1 file**, và `wc -l` của rule vẫn `<= 95`. Đụng `finding.md` ⇒ đã lấn sang T-32, đừng nhận.
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Mỗi mục 🔴 có thêm **một** vế `**Đóng khi.**` — không mục nào mất chữ nào | `git show <sha vừa commit> -- finding.md \| grep -c '^-[^-]'` → `0` | [finding.md](finding.md) › từng mục `### F-xx` còn 🔴 |
| Mục chưa nghĩ ra lệnh dùng vế thoát, không bịa lệnh | — | [finding.md](finding.md) › mục kiến trúc chờ owner (`F-07` `F-08` `F-12`) |

**Đạt khi:** `git show --stat <sha vừa commit>` ra **đúng 1 file**, chỉ có dòng `+`. Bảng khuôn ở rule bị đụng ⇒ task này đang sửa luật, trả lại.

### owner-T-34

**Nhà của câu hỏi chất lượng thứ ba — nội dung có ĐÚNG ĐỂ XÂY không** · ai ký: 👤 *(thêm một câu hỏi chất lượng áp cho mọi lane mọi pha; rule §1.2b)*

| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Nhà mới: sáu trục nội dung N1–N6, thủ tục chấm bốn bước, ba nhãn tại chỗ, ranh giới | `git show 95fb906 -- quality/01-chat-luong-noi-dung.md` | `quality/01-chat-luong-noi-dung.md` **(mới)** › cả file, §1–§8 |
| Hàng `quality` ở bản đồ §2 **gộp** hai nhà vào một hàng, không thêm dòng | `git show 95fb906 -- CLAUDE.md` | [CLAUDE.md](CLAUDE.md) §2 › hàng `Chất lượng đầu ra` |
| Header guideline trỏ sang câu hỏi thứ ba, và ngày khai đổi theo | `git show 95fb906 -- quality/00-guideline-chat-luong.md` | [quality/00-guideline-chat-luong.md](quality/00-guideline-chat-luong.md) › blockquote đầu file, dòng `Cập nhật` |

**Đạt khi:** đúng **5** file trong commit (ba file trên + `task.md` + `finding.md`); `CLAUDE.md` đổi đúng **1** dòng và §2 vẫn **18** dòng; không câu nào của guideline §3 §4 §5 bị chép sang nhà mới — `grep -c 'dùng được lạnh' quality/01-chat-luong-noi-dung.md` ra `0`.

### owner-T-37

**Gỡ ⚠️ mồ côi ở ô Đ4 lane BA** · ai ký: 🤖 *(gỡ một dấu đã sai, không đổi mệnh đề nào)*

| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Bỏ `⚠️` trước `project_preparation/00-scope.md §4.4`, giữ nguyên con trỏ và cả câu còn lại | `git show c6155eb -- design/BA/04-yeu-cau.md` | [design/BA/04-yeu-cau.md](design/BA/04-yeu-cau.md) › §1 bảng đầu ra › hàng `Đ4`, dòng 22 |

**Đạt khi:** ~~đúng **3** file trong commit~~ — **ô này hết đúng, và lý do là một finding**: hunk của
`design/BA/04-yeu-cau.md` bị một phiên song song commit hộ ở `c6155eb` *trước khi* phiên BA kịp stage, nên
task này ra **hai** commit: `c6155eb` (một dòng của lane BA, message khai lane NON-CODE) + commit của phiên
BA (`task.md` + `finding.md`). Ca sống thứ hai của [F-07](finding.md#f-07). Ba vế còn lại **vẫn đo được và
đã xanh**: diff lane BA chỉ đụng **một** dòng (`git show c6155eb -- design/BA/04-yeu-cau.md`);
[F-31](finding.md#f-31) chuyển `✅ ĐÓNG` kèm vế `**Bài học giữ lại:**`; ba vế biên nhận ra `0 · 1 · 4`.


### owner-T-36

**Rule mỏng chở nhịp chất lượng nội dung vào lane code** · ai ký: 🤖 *(rule mới, không đụng nhà thật nào)*

| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Rule mới: `paths:` phủ nơi nội dung ra đời, thân chỉ trỏ sang `quality/01` | — | `.claude/rules/chat-luong-noi-dung.md` ⚠️ *(T-36 tạo)* › frontmatter + thân |
| Vế thứ 6 của quy trình mở lane: mở lane ⇒ nới `paths:` rule chất lượng nội dung | `git diff -- .claude/rules/quan-ly-du-an.md` | [.claude/rules/quan-ly-du-an.md](.claude/rules/quan-ly-du-an.md) › mục 6 |

**Đạt khi:** đúng **4** file trong commit (hai file trên + `task.md` + `finding.md`); rule mới không chứa dòng tiêu đề trục `N<i>` nào; [F-43](finding.md#f-43) chuyển `✅ ĐÓNG` kèm vế `**Bài học giữ lại:**`.


### owner-T-35

**Cưỡng chế sáu probe bằng hook** · ai ký: 👤 *(hook chạy ở mọi phiên)*

| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Hook `Stop` nhắc khi câu trả lời thiếu phiếu sáu dòng | `git diff -- .claude/settings.json` | `.claude/settings.json` › khối `hooks` › `Stop` |

**Đạt khi:** `.claude/settings.json` parse được bằng `json.load`; hook **nhắc**, không chặn — không có `exit 2` nào trong khối vừa thêm.

### owner-T-33

**Nhà của ràng buộc kiến trúc ẩn (§6.8)** · ai ký: 👤 *(gộp một hàng con trỏ ở `CLAUDE.md` §2 — rule §1.2b: sửa luật ở §2–§8)*

| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Năm ràng buộc kiến trúc ẩn + dấu hiệu phải xem lại có nhà thật, thôi sống trong prompt bàn giao | — | `quality/06-rang-buoc-kien-truc.md` **(mới)** › cả file |
| Câu *"file này không sở hữu cái gì"* của checklist nay **trỏ** §6.8 sang nhà mới — nối vào dòng có sẵn, không đẻ dòng | — | [quality/05-checklist.md](quality/05-checklist.md) §4 › dòng ranh giới có sẵn |
| Bản đồ *một sự thật một nhà* có hàng cho §6.8, đổi lấy **một** hàng cũ bị gộp | — | [CLAUDE.md](CLAUDE.md) §2 › **đúng một hàng** bảng |

**Đạt khi:** `git show --stat <sha vừa commit>` ra **đúng 3 file**; `wc -l < quality/05-checklist.md` vẫn `<= 75` và mục §2 của `CLAUDE.md` vẫn **18 dòng**. Đụng `project_preparation/prompt-fullstack.md` ⇒ phiên đang sửa nguồn bàn giao thay vì dựng nhà, trả lại.
...
**ĐẠT khi:** owner ký tận mắt — đây là thứ thuộc quyền owner (`CLAUDE.md` §7). · **Đã thử làm đỏ:** ✅ 2026-08-24 — ba vế ô biên nhận, trên bản sao ở scratchpad: `test -e` trên tên file không tồn tại ⇒ kêu · thêm lại ⚠️ vào dòng `CLAUDE.md` §2 ⇒ `grep -c '⚠️'` ra `1` (bản thật `0`). **Vế `grep -c '^| ' >= 8` KHÔNG đỏ được có ý nghĩa:** file 8 dòng bảng toàn chữ *rác*, không một chữ về giá, cũng ra `8` ⇒ xanh — ngưỡng dưới đúng loại [F-13](finding.md#f-13) tố, mã đó đang 🔴 và sở hữu việc sửa, không đẻ mã mới. · **Hàng *Một nhà* đang ĐỎ GIẢ:** lệnh ra `3` file, nhưng hai trong ba là `task.md` (khớp vì chính dòng T-05 chứa chữ *phạm vi bán*) và `prompt-fullstack.md` §9.3 (bảng ca test, cố ý hardcode). Lệnh quét cả repo trong khi T-05 chỉ chạm 3 file — đúng luật mà [F-29](finding.md#f-29)/[F-30](finding.md#f-30) đã rút; không sửa lệnh ở đây vì đó là sửa đề bài cho khớp việc mình vừa làm. Bản dẫn xuất thật đã có lệnh cưỡng chế ở [F-31](finding.md#f-31).
...
**ĐẠT khi:** cổng (a) rỗng **và** `git show <sha vừa commit> -- finding.md` không có dòng `-` nào (không xoá nội dung cũ để lấy vế mới cho nhanh). · **Đã thử làm đỏ:** ⬜ chưa
...
| Một nhà | vế mới **không** chép lại mệnh đề hay lệnh tái hiện đã có trong chính mục đó | `git show <sha vừa commit> -- finding.md \| grep '^+' \| grep -c 'Lệnh tái hiện'` → `0` | `>= 1` ⇒ vế mới đang là bản sao của vế cũ, và hai bản sẽ lệch nhau ở lần sửa đầu tiên |

**ĐẠT khi:** ba hàng xanh **và** cổng (c) đỏ thật khi gỡ vế của một mục. · **Đã thử làm đỏ:** ⬜ chưa

### cl-T-34

**`quality/01-chat-luong-noi-dung.md`** · sàn **T2** · ✅ đo tới **T2** ngày 2026-08-25, chưa diễn tập T3

| Trục §4 | Mệnh đề phải đúng | Lệnh soi (≥ 2 file, khác ô biên nhận) | Đỏ khi |
|---|---|---|---|
| Một nhà | sáu trục nội dung chỉ có **một** nhà; guideline **không** mọc bản thứ hai | `grep -rln 'Không có số mồ côi' --include='*.md' .` → đúng **1** file | ra `>= 2` ⇒ đã chép sang guideline hoặc tầng pha thay vì trỏ ([guideline §7](quality/00-guideline-chat-luong.md) cấm) |
| Đúng | mỗi trục N1–N6 là trục guideline §4 **chưa có**, không phải bản diễn giải lại năm trục cũ | `grep -c 'Rẻ để đọc' quality/01-chat-luong-noi-dung.md` → `0`, và `grep -c 'Rẻ để đọc' quality/00-guideline-chat-luong.md` → `>= 1` | số thứ nhất `>= 1` ⇒ nhà mới đã kéo trục của guideline sang, hai bản sẽ lệch ở lần sửa đầu tiên |
| Dùng được lạnh | phiên mới **chỉ** nạp nhà mới là chấm được một đầu ra ĐẠT/KHÔNG ĐẠT mà không hỏi thêm | diễn tập T3: mở phiên mới, nạp đúng `quality/01-chat-luong-noi-dung.md`, giao việc *"chấm `design/BA/04-yeu-cau.md`"* | nó phải hỏi *"chấm theo tiêu chí nào"*, hoặc chấm bằng cách đọc lại thay vì chạy sáu probe ⇒ §5 chưa đủ rõ để làm theo |

**ĐẠT khi:** ba hàng xanh. Hàng 1 và 2 đã chạy 2026-08-25 (ra `1` file; ra `0` và `1`). Hàng 3 chưa diễn tập. · **Đã thử làm đỏ:** ✅ 2026-08-25 — xoá vế `Đỏ khi` của N6 thì phép đếm ra `6` trục trên `5` vế âm (đỏ đúng như khai); chép một hàng bảng năm trục vào thì phép đếm chép-thay-vì-trỏ ra `1` thay vì `0`. Khôi phục bằng bản sao, cả hai về xanh.

### cl-T-37

**`design/BA/04-yeu-cau.md`** · sàn **T2** · ✅ đo 2026-08-25 — ba hàng xanh

| Trục §4 | Mệnh đề phải đúng | Lệnh soi (≥ 2 file, khác ô biên nhận) | Đỏ khi |
|---|---|---|---|
| Đúng | ⚠️ trong repo chỉ còn đứng ở chỗ **thật sự chưa có**; §4.4 đã có nhà từ T-05 | `test -e project_preparation/00-scope.md` không kêu, và `sed -n '/^### 4\.4 /,/^## §5/p' project_preparation/00-scope.md | grep -c '^| Suất'` ra `>= 2` | nhà thật rỗng mà vẫn gỡ ⚠️ ⇒ đổi một con trỏ sai thành một con trỏ **nói dối theo hướng ngược lại**, tệ hơn bản cũ |
| Dùng được lạnh | phiên BA chưa từng đọc `00-scope.md`, nhìn ô Đ4 là biết tra §4.4 chứ không tự chốt lại | đọc ô Đ4: phải còn **đủ** đường dẫn `project_preparation/00-scope.md` **và** mục `§4.4` ([CLAUDE.md §4](CLAUDE.md) — trích dẫn đủ nhà) | gỡ ⚠️ kèm gỡ luôn số mục ⇒ phiên sau phải tự dò trong cả file, và dò trượt thì tự chốt lại |
| Đủ | không còn ⚠️ mồ côi nào khác trỏ vào nhà đã có, trong **cả** `design/BA/**` | `grep -rn '⚠️' design/BA/ | grep -c '00-scope'` ra `0`, và `grep -c '⚠️ chưa mở' CLAUDE.md` ra `4` *(bốn lane chưa mở — ⚠️ đúng chỗ, không được đụng)* | phiên chỉ vá đúng dòng 22 mà bỏ sót một ⚠️ cùng loại ⇒ [F-31](finding.md#f-31) đóng non, và mã đã đóng thì không ai mở lại |

**ĐẠT khi:** ba hàng xanh **và** bốn cổng lane BA ở [design/BA/04-yeu-cau.md §2](design/BA/04-yeu-cau.md) còn xanh. · **ĐÃ ĐO 2026-08-25:** ba hàng xanh (`test -e` không kêu · `^| Suất` ra `4` · ô Đ4 còn đủ đường dẫn **và** `§4.4` · `grep -rn '⚠️' design/BA/ | grep -c '00-scope'` ra `0` · `grep -c '⚠️ chưa mở' CLAUDE.md` ra `4`), bốn cổng lane BA xanh (`test -e` không kêu · `^| Đ[0-9]` ra `4` · `head -6 .claude/rules/lane-ba.md` in `paths: design/BA/**` · `grep -c 'design/BA' CLAUDE.md` ra `2`). · **Đã thử làm đỏ:** ✅ 2026-08-25, trên bản sao ở scratchpad — trả ⚠️ về dòng 22 ⇒ vế 1 ra `1`; thay cả cụm đường dẫn + `§4.4` bằng chữ *nhà thật đã có* ⇒ vế 2 ra `0` **và** trục *Dùng được lạnh* ra `0`; đổi `| Đ4 ` thành `| D4 ` ⇒ vế 3 ra `3`. Bản thật vẫn `0 · 1 · 4`.


### cl-T-36

**`.claude/rules/chat-luong-noi-dung.md`** · sàn **T2** · ⬜ chưa đo — task chưa làm

| Trục §4 | Mệnh đề phải đúng | Lệnh soi (≥ 2 file, khác ô biên nhận) | Đỏ khi |
|---|---|---|---|
| Đúng | rule mới **trỏ** sáu trục chứ không **chép**: nhà của N1–N6 vẫn là [quality/01](quality/01-chat-luong-noi-dung.md) | `grep -c '§3 §4' .claude/rules/chat-luong-noi-dung.md` ra `>= 1`, và `grep -c 'N1 · Ánh xạ' quality/01-chat-luong-noi-dung.md` ra `1` | tên trục xuất hiện ở hai file ⇒ đổi một trục phải sửa hai chỗ, chỗ thứ hai sẽ quên (đúng gốc [F-04](finding.md#f-04)) |
| Dùng được lạnh | phiên chưa từng đọc `quality/01`, bị rule nạp vào là biết ngay **chạy gì · khi nào · đỏ thì làm gì**, không phải mở file thứ hai mới biết đây là việc bắt buộc | đọc thân rule: phải có đủ ba mẩu đó **và** đường dẫn đầy đủ `quality/01-chat-luong-noi-dung.md` §3 §4 §5 | thân rule chỉ nói *"tuân theo quality/01"* ⇒ phiên vội đọc lướt rồi bỏ, và rule thành trang trí |
| Đủ | `paths:` phủ **cả bốn** lane chưa mở (DB/BE/FE/DEVOPS), không chỉ lane đang mở | `grep -c '^6\. \*\*' .claude/rules/quan-ly-du-an.md` ra `1`, và `grep -c 'chưa mở' CLAUDE.md` ra `4` | mẫu chỉ phủ `design/**` ⇒ đúng lỗi [F-43](finding.md#f-43) vừa đóng, tái phát ở lane kế mở ra |

**ĐẠT khi:** ba hàng xanh **và** `paths:` của rule mới **không** phải `"**"` — nạp ở mọi phiên là thuế mọi phiên ([CLAUDE.md §8](CLAUDE.md)), và luật phiên nào cũng thấy là luật phiên nào cũng bỏ qua. · **Đã thử làm đỏ:** ⬜ chưa


### cl-T-35

**`.claude/settings.json`** · sàn **T2** · ⬜ chưa đo — task chưa làm

| Trục §4 | Mệnh đề phải đúng | Lệnh soi (≥ 2 file, khác ô biên nhận) | Đỏ khi |
|---|---|---|---|
| Đúng | hook nhắc đúng thứ [quality/01](quality/01-chat-luong-noi-dung.md) §5 bước 3 đòi, không phải một khuôn phiếu thứ hai | `grep -c 'N1' .claude/settings.json` → `>= 1`, và `grep -c 'N<i> · ĐẠT' quality/01-chat-luong-noi-dung.md` → `>= 1` | hook dò một chuỗi không có trong §5 ⇒ nó cưỡng chế một khuôn không ai viết theo |
| Dùng được lạnh | phiên mới bị hook nhắc là **biết phải làm gì tiếp** mà không cần mở file luật | đọc câu nhắc: nó phải chứa đường dẫn `quality/01-chat-luong-noi-dung.md` §5 | câu nhắc chỉ nói *"thiếu phiếu chất lượng"* ⇒ phiên sau tắt hook thay vì làm theo |

**ĐẠT khi:** hai hàng xanh **và** hook không chặn phiên nào — nhắc mà chặn thì nó bị tắt, và tắt rồi thì nó không còn là cơ chế gì nữa ([guideline §5](quality/00-guideline-chat-luong.md) luật 3). · **Đã thử làm đỏ:** ⬜ chưa

### cl-T-33

**`quality/06-rang-buoc-kien-truc.md`** · sàn **T3** · ⬜ chưa đo

| Trục §4 | Mệnh đề phải đúng | Lệnh soi (≥ 2 file, khác ô biên nhận) | Đỏ khi |
|---|---|---|---|
| Đúng | năm ràng buộc là **của §6.8**, không phải năm cái phiên tự nghĩ ra cho đủ số | `for k in '1 instance' 'hàng đợi' 'polling 20s' '200 món' '1 VPS'; do grep -qF "$k" quality/06-rang-buoc-kien-truc.md && grep -qF "$k" project_preparation/prompt-fullstack.md \|\| echo "LỆCH: $k"; done` → rỗng | in ra một dòng `LỆCH` ⇒ nhà mới bỏ sót một ràng buộc, hoặc thêm một ràng buộc không có ở nguồn — cả hai đều làm nó hết là bản dịch của §6.8 |
| Một nhà | đúng **hai** file trong repo nói tới ràng buộc này: nguồn bàn giao (ngoài, chỉ đọc) và nhà mới | `grep -rln '1 instance' --include='*.md' --exclude-dir=reference --exclude=task.md --exclude=finding.md .` → **đúng 2** | ra `1` ⇒ chưa có nhà; ra `>= 3` ⇒ đã đẻ bản chép thứ ba (thường là chép luôn vào [quality/05-checklist.md](quality/05-checklist.md) cho "tiện tra"), và hai bản sẽ lệch ngay ở lần sửa đầu tiên |
| Dùng được lạnh | phiên mới **chỉ** nạp nhà mới là **dừng lại được** trước một việc phá ràng buộc | diễn tập T3: mở phiên mới, nạp đúng `quality/06-rang-buoc-kien-truc.md`, giao việc *"trạm bếp chậm, thêm bản BE thứ hai cho nhẹ tải"* | nó làm tiếp mà không nhắc SSE giữ kết nối trong bộ nhớ process ⇒ file mới **ghi ra** ràng buộc nhưng chưa nói được **dấu hiệu nào bắt phải xem lại**, tức mới đạt T1 chứ chưa T3 |

**ĐẠT khi:** ba hàng xanh **và** không ràng buộc nào bỏ trống ô *dấu hiệu phải xem lại* — ràng buộc không có dấu hiệu là một lời khuyên, không phải một cổng. Việc **gắn** dấu hiệu vào cổng cưỡng chế là task riêng **chưa mở**; thiếu nó không làm hàng nào ở đây đỏ. · **Đã thử làm đỏ:** ⬜ chưa
