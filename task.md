...
| Trục §4 | Mệnh đề phải đúng | Lệnh soi (≥ 2 file, khác ô biên nhận) | Đỏ khi |
|---|---|---|---|
| Một nhà | nội dung **7 vế §6.5** chỉ có một nhà trong repo là checklist; `prompt-fullstack.md` là prompt bàn giao (nguồn ngoài, chỉ đọc) | `grep -rln 'happy path' --include='*.md' --exclude-dir=reference .` | ra `>= 3` file ⇒ có bản chép thứ ba; hoặc kết quả **không** có `quality/05-checklist.md` ⇒ chưa có nhà |
...
**ĐẠT khi:** checklist **tiếp nối** thang [guideline §3](quality/00-guideline-chat-luong.md) ở T1/T2 chứ không thay nó (guideline §7 câu cuối). · **Đã thử làm đỏ:** ✅ 2026-08-23 — `awk` 7 vế in ra `3` trên bản thiếu 4 vế và `7` trên bản đủ; `grep -rln 'happy path'` ra `1` khi chưa có nhà và `3` khi dựng thêm bản chép thứ ba (thử trên bản giả ở scratchpad, repo không bị chạm)
...
**Lệnh (e) chạy được** · sàn **T2** · ⬜ chưa đo
...
**ĐẠT khi:** hai hàng xanh — (e) là thứ [T-27](#sổ-task) dựa vào để biết đã sửa hết chưa. · **Đã thử làm đỏ:** ⬜ chưa
