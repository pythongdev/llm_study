...
| §3.2 rút còn 4 dòng trỏ lên `00-scope.md`; §9.3 khai 10 ca giá là **dẫn xuất** từ §4.2 | `git log -p -1 -- project_preparation/prompt-fullstack.md` | [prompt-fullstack.md](project_preparation/prompt-fullstack.md) §3.2 + §9.3 › hai chỗ |
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Nhà mới: sáu trục nội dung N1–N6, thủ tục chấm bốn bước, ba nhãn tại chỗ, ranh giới | `git show HEAD -- quality/01-chat-luong-noi-dung.md` | `quality/01-chat-luong-noi-dung.md` **(mới)** › cả file, §1–§8 |
| Hàng `quality` ở bản đồ §2 **gộp** hai nhà vào một hàng, không thêm dòng | `git show HEAD -- CLAUDE.md` | [CLAUDE.md](CLAUDE.md) §2 › hàng `Chất lượng đầu ra` |
| Header guideline trỏ sang câu hỏi thứ ba, và ngày khai đổi theo | `git show HEAD -- quality/00-guideline-chat-luong.md` | [quality/00-guideline-chat-luong.md](quality/00-guideline-chat-luong.md) › blockquote đầu file, dòng `Cập nhật` |
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Rule mới: `paths:` phủ nơi nội dung ra đời, thân chỉ trỏ sang `quality/01` | `git show --stat HEAD -- .claude/rules/chat-luong-noi-dung.md` | `.claude/rules/chat-luong-noi-dung.md` ⚠️ *(T-36 tạo)* › frontmatter + thân |
...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Năm ràng buộc kiến trúc ẩn + dấu hiệu phải xem lại có nhà thật, thôi sống trong prompt bàn giao | `git show --stat HEAD -- quality/` | `quality/06-rang-buoc-kien-truc.md` **(mới)** › cả file |
| Câu *"file này không sở hữu cái gì"* của checklist nay **trỏ** §6.8 sang nhà mới — nối vào dòng có sẵn, không đẻ dòng | `git show HEAD -- quality/05-checklist.md` | [quality/05-checklist.md](quality/05-checklist.md) §4 › dòng ranh giới có sẵn |
| Bản đồ *một sự thật một nhà* có hàng cho §6.8, đổi lấy **một** hàng cũ bị gộp | `git show HEAD -- CLAUDE.md` | [CLAUDE.md](CLAUDE.md) §2 › **đúng một hàng** bảng |

**Đạt khi:** `git show --stat HEAD` ra **đúng 3 file**; `wc -l < quality/05-checklist.md` vẫn `<= 75` và mục §2 của `CLAUDE.md` vẫn **18 dòng**. Đụng `project_preparation/prompt-fullstack.md` ⇒ phiên đang sửa nguồn bàn giao thay vì dựng nhà, trả lại.
