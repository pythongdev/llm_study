...
| Thay đổi cái gì | Câu lệnh để thấy thay đổi | Ở đâu |
|---|---|---|
| Rule mới: `paths:` phủ nơi nội dung ra đời, thân chỉ trỏ sang `quality/01` | `git show --stat HEAD -- .claude/rules/chat-luong-noi-dung.md` | [.claude/rules/chat-luong-noi-dung.md](.claude/rules/chat-luong-noi-dung.md) › frontmatter + thân |
...
| Đủ | `paths:` phủ **cả bốn** lane chưa mở (DB/BE/FE/DEVOPS), không chỉ lane đang mở | `grep -c 'chat-luong-noi-dung' .claude/rules/quan-ly-du-an.md` ra `>= 1`, và `grep -c 'chưa mở' CLAUDE.md` ra `4` | mẫu chỉ phủ `design/**` ⇒ đúng lỗi [F-43](finding.md#f-43) vừa đóng, tái phát ở lane kế mở ra |
