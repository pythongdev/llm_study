...
**Cột `File chạm`.** Ghi file mà mệnh đề đang làm sai, cộng file mà `**Cách sửa đề xuất.**` sẽ động vào —
**không** ghi file chỉ nhắc qua để lấy bối cảnh. Đây là cột [CLAUDE.md §3](CLAUDE.md) bước 3 grep trước khi gõ, nên
**dòng mới thiếu cột này là finding vô hình**: phiên sửa đúng file đó sẽ giẫm lại nó mà không lệnh nào kêu.

---

| # | Mệnh đề đang sai | Phát hiện lúc | Trạng thái | Task đóng nó | File chạm |
|---|---|---|---|---|---|
| [F-01](#f-01) | Hai hệ tên cho cùng một trục chia việc: `reference/task.md` gọi *Giai đoạn*, `prompt-fullstack.md` §7 gọi *Pha* | T-01 · 2026-08-22 | ✅ ĐÓNG 2026-08-22 | T-01 | `reference/task.md` |
| [F-02](#f-02) | `project_preparation/prompt-fullstack.md` trỏ tới 8 đường dẫn không tồn tại, không đường nào đánh ⚠️ | T-01 · 2026-08-22 | 🔴 MỞ | [T-10](task.md) | `CLAUDE.md` `project_preparation/prompt-fullstack.md` `quality/05-checklist.md` `task.md` |
| [F-03](#f-03) | `paths:` của `.claude/rules/quan-ly-du-an.md` không khớp `CLAUDE.md`, nên phiên sửa `CLAUDE.md` §1 để mở lane không được nạp quy trình mở lane | T-01 · 2026-08-22 | ✅ ĐÓNG 2026-08-22 | T-11 | `.claude/rules/quan-ly-du-an.md` `CLAUDE.md` `finding.md` `task.md` |
| [F-04](#f-04) | Đặc tả *một ô của dòng task viết cái gì* sống ở **hai** nhà — rule §1.1 và `huong-dan-viet-task-md.md` Phần V — một sự thật đổi thì phải sửa hai file, không lệnh nào bắt được khi chỉ sửa một | rà guideline · 2026-08-22 | 🔴 MỞ | [T-12](task.md) | `.claude/rules/quan-ly-du-an.md` `CLAUDE.md` `project_preparation/huong-dan-viet-task-md.md` `task.md` |
| [F-05](#f-05) | Bốn lệnh tự rà ở rule §5.2 không đáng tin: dòng vỡ cột thì chúng đọc nhầm ô (T-05 có 11 cột vì dấu ngăn chưa thoát), và mẫu dò finding lỏng nên báo nhầm mã không tồn tại | T-12 · 2026-08-22 | 🔴 MỞ | [T-13](task.md) | `.claude/rules/quan-ly-du-an.md` `task.md` |
| [F-06](#f-06) | `finding.md` bị chèn thừa một khối `F-04` lai (mở đầu F-04, thân F-02) và một bản sao y hệt `### F-03`; hai mục cùng mã tồn tại 60 dòng mà không lệnh nào bắt được | T-11 · 2026-08-22 | 🔴 MỞ | [T-16](task.md) | `.claude/rules/quan-ly-du-an.md` `finding.md` `task.md` |
| [F-07](#f-07) | Hai phiên chạy cùng lúc trên một repo: [CLAUDE.md §3](CLAUDE.md) bước 4 bắt `commit` hoặc `git stash` khi cây bẩn, nhưng cây bẩn của **phiên khác** trông y hệt cây bẩn của chính mình ⇒ chạy đúng luật là nuốt việc dở của phiên kia | kiểm thử ngược · 2026-08-22 | 🔴 MỞ | ⚠️ chưa có task — **kiến trúc, chờ owner chốt** | `CLAUDE.md` `finding.md` `task.md` |
| [F-08](#f-08) | `CLAUDE.md` §8 đặt trần **mỗi mục ≤ 14 dòng** nhưng §1 đang 20 dòng và §2 đang 17 — §8 chỉ nêu lệnh kiểm `wc -l` toàn file và `awk length>400`, không lệnh nào kiểm trần từng mục | kiểm thử ngược lượt 2 · 2026-08-22 | ✅ ĐÓNG 2026-08-25 | ⚠️ không có task — vá ngoài sổ ở `c6155eb`+`b334899`, đóng ở phiên NON-CODE 2026-08-25 | `CLAUDE.md` |
| [F-09](#f-09) | Cách sửa mà [F-05](#f-05) đề xuất **không làm lệnh đếm cột xanh được**: `awk -F'\|'` đếm cả dấu `\|` đã thoát, nên T-05 và T-11 — hai dòng đã thoát **đúng luật** §1.1 — vẫn bị báo vỡ cột vĩnh viễn; và vế *T-05 chưa thoát dấu* của F-05 nay đã hết đúng | rà bộ khung · 2026-08-22 | 🔴 MỞ | ⚠️ chưa có task — sửa kèm khi làm [T-13](task.md) | `.claude/rules/quan-ly-du-an.md` `finding.md` `task.md` |
| [F-10](#f-10) | Luật *sửa nội dung ⇒ đổi ngày trong cùng commit* ở [CLAUDE.md §2](CLAUDE.md) không có lệnh nào kiểm; bốn file `.md` đang khai ngày khác `git log`, trong đó `project_preparation/prompt-fullstack.md` lệch 2 ngày — không ai thấy | rà bộ khung · 2026-08-22 | 🔴 MỞ | ⚠️ chưa có task | `.claude/rules/quan-ly-du-an.md` `CLAUDE.md` `project_preparation/prompt-fullstack.md` `task.md` |
| [F-11](#f-11) | Vòng lặp con trỏ §5.1 **tự bắt chính mình**: chạy trên `.claude/rules/quan-ly-du-an.md` luôn in một dòng `TRỎ HỤT` rác lấy từ chính đoạn code của lệnh ⇒ vế 3 của định nghĩa XONG (§3) không bao giờ xanh được trên file luật | rà bộ khung · 2026-08-22 | 🔴 MỞ | ⚠️ chưa có task | `.claude/rules/quan-ly-du-an.md` |
| [F-12](#f-12) | Cả bộ khung đặt cưỡng chế lên `paths:` của `.claude/rules/**` — nhưng **không lệnh nào trong repo chứng minh được một rule có thật sự được nạp**; `test -e` chỉ chứng minh file tồn tại, không chứng minh nó tới được phiên | rà bộ khung · 2026-08-22 | 🔴 MỞ | ⚠️ chưa có task — **kiến trúc, chờ owner chốt** | `.claude/rules/bao-cao-thay-doi.md` `CLAUDE.md` |
| [F-13](#f-13) | Không biên nhận nào trong repo đo **chất lượng**: cả 16 dòng `task.md` dừng ở *tồn tại + hình dạng*, không dòng nào khai **đỏ khi** — nên ngưỡng dưới kiểu `grep -c '^- '` ra `>= 3` xanh **vĩnh viễn**, kể cả với ba dòng rác | owner nêu · 2026-08-23 | 🔴 MỞ | [T-20](task.md) + [T-21](task.md) | `.claude/rules/quan-ly-du-an.md` `quality/00-guideline-chat-luong.md` `task.md` |
| [F-14](#f-14) | Luật sàn ở [guideline §3](quality/00-guideline-chat-luong.md) khai **T3 bắt buộc** cho *file mà phiên khác sẽ nạp* — trong repo chỉ-có-tài-liệu này **mọi** `.md` đều là file phiên khác nạp, nên sàn T3 rơi vào 21/23 dòng task và cột `Chất lượng` lọc được đúng bằng không, cùng kiểu hỏng với *đánh 👤 cho mọi dòng* mà rule §1.2b đã cấm | T-24 · 2026-08-23 | 🔴 MỞ | ⚠️ chưa có task — sửa luật chất lượng là việc owner chốt | `.claude/rules/quan-ly-du-an.md` `CLAUDE.md` `Makefile` `finding.md` `quality/00-guideline-chat-luong.md` `task.md` |
| [F-15](#f-15) | 50/61 lệnh ở cột `Câu lệnh để thấy thay đổi` của §Owner kiểm tra ghi `HEAD`; `HEAD` di chuyển sau mỗi commit nên với task **đã xong** lệnh in ra rỗng (`owner-T-02`, cả 3 lệnh) hoặc in ra **diff của task khác** (`owner-T-14` `owner-T-22` đang chiếu diff của T-24) | T-02 rà pipeline · 2026-08-23 | 🔴 MỞ | [T-27](task.md) | `.claude/rules/quan-ly-du-an.md` `CLAUDE.md` `task.md` |
| [F-16](#f-16) | Lệnh **(e)** ở [bao-cao-thay-doi.md](.claude/rules/bao-cao-thay-doi.md) §3 — lệnh **duy nhất** bắt được [F-15](#f-15) — không chạy được: thân nó là hai dòng chú thích cộng `git log --oneline -3 -- <path>` với `<path>` là chỗ trống, và T-14 chỉ đưa (b) (c) (d) vào bộ bắt buộc | T-02 rà pipeline · 2026-08-23 | ✅ ĐÓNG 2026-08-24 | T-26 | `.claude/rules/bao-cao-thay-doi.md` `task.md` |
| [F-17](#f-17) | [CLAUDE.md §2](CLAUDE.md) trao cho `git log` vai nhà duy nhất của *"ai sửa file nào"*, nhưng không lệnh nào đối chiếu message với `--stat`: `c386219` khai sửa `.claude/rules/quan-ly-du-an.md` trong khi commit đó chỉ đụng `finding.md` và `task.md` | T-02 rà pipeline · 2026-08-23 | 🔴 MỞ | [T-15](task.md) | `.claude/rules/quan-ly-du-an.md` `CLAUDE.md` `finding.md` `task.md` |
| [F-18](#f-18) | Không cơ chế nào bắt *task đã xong trong git mà dòng sổ còn mở*: T-02 commit đủ đầu ra ở `4851d17` với cả năm vế biên nhận xanh, dòng vẫn `\| **T-02** 🔺` — nên bước 2 của [CLAUDE.md §3](CLAUDE.md) dò đúng vào nó và phiên sau mở ra làm lại việc đã xong | T-02 rà pipeline · 2026-08-23 | 🔴 MỞ | [T-30](task.md) | `CLAUDE.md` `design/BA/04-yeu-cau.md` `task.md` |
| [F-19](#f-19) | Một cơ chế đang hỏng được ghi **ngoài sổ lỗi**: `design/BA/04-yeu-cau.md` §4 mục 2 (*"Đóng T-02 xong thì vế 5 tự đỏ"*) mô tả đủ chuyện đóng T-02 làm lệnh §5.2b in `LANE NỬA VỜI: ba`, nhưng `grep -c 'LANE NỬA VỜI' finding.md` ra `0` và định tuyến §1 không dẫn lane NON-CODE tới `design/BA/**` | T-02 rà pipeline · 2026-08-23 | ✅ ĐÓNG 2026-08-27 | T-28 | `.claude/rules/bao-cao-thay-doi.md` `.claude/rules/quan-ly-du-an.md` `CLAUDE.md` `design/BA/04-yeu-cau.md` `finding.md` |
...
| [F-30](#f-30) | Biên nhận thứ nhất của [T-26](task.md) — `grep -c '<path>' .claude/rules/bao-cao-thay-doi.md` ra `0` — quét **cả file** trong khi phạm vi T-26 chỉ là §3: `<path>` ở §1 cột `Xem diff` và §2 cột `Câu lệnh để thấy thay đổi` là placeholder văn xuôi **hợp lệ, phải giữ**, nên ô này ra `2` kể cả khi (e) đã vá đúng — đỏ vĩnh viễn, và cách duy nhất làm nó xanh là lấn phạm vi | T-26 · 2026-08-24 | ✅ ĐÓNG 2026-08-24 | ⚠️ đóng ngay trong phiên vá — không đẻ task | `.claude/rules/bao-cao-thay-doi.md` `quality/00-guideline-chat-luong.md` `task.md` |
| [F-31](#f-31) | Hai bản **dẫn xuất** từ `project_preparation/00-scope.md` không có cơ chế nào canh: `prompt-fullstack.md` §9.3 giữ 10 số giá tự khai *"hợp đồng với chủ quán"* mà chỉ khai nguồn bằng câu chữ, và `design/BA/04-yeu-cau.md` dòng 22 còn đánh ⚠️ cho `00-scope.md` §4.4 trong khi nhà đã có | T-05 · 2026-08-24 | ✅ ĐÓNG 2026-08-25 | [T-37](task.md) | `CLAUDE.md` `Makefile` `design/BA/04-yeu-cau.md` `project_preparation/00-scope.md` `project_preparation/prompt-fullstack.md` `task.md` |
| [F-32](#f-32) | `.claude/rules/bao-cao-thay-doi.md` §3 khai *"cả năm lệnh (a) (b) (c) (d) (e) đều bắt buộc"* nhưng dán nhãn năm cổng bằng **hai ký pháp** và **sai thứ tự** — (a) (b) (c) (e) là `# x.` trong khối bash, (d) là `**d.**` ngoài khối và đứng sau (e) — nên không lệnh nào đếm nổi đủ năm, mọi phép đếm hiển nhiên ra `4`. **Bản mở của mã này tự trượt vào đó**: nó kết luận (d) là *"lệnh ma chưa từng tồn tại"* và đề nghị **bỏ** một cổng đang chạy tốt từ `b520e7c` | T-05 · 2026-08-24 | ✅ ĐÓNG 2026-08-25 | ⚠️ đóng ngay trong phiên vá — không đẻ task | `.claude/rules/bao-cao-thay-doi.md` |
| [F-33](#f-33) | `quality/05-checklist.md` tự khai là **nhà duy nhất** của câu hỏi *"task này XONG chưa"* cho task có code, nhưng cả bảy vế §1 chỉ đo **vệ sinh của đầu ra** — không vế nào hỏi đầu ra **có phải thứ ô `Task` yêu cầu không**; dòng 57 còn đẩy việc đó sang câu hỏi TỐT | rà quality/05-checklist.md · 2026-08-24 | 🔴 MỞ | ⚠️ chưa có task | `quality/00-guideline-chat-luong.md` `quality/05-checklist.md` |
| [F-34](#f-34) | Biên nhận `grep -c '^- \[ \]'` **= đúng 7, không hơn không kém** bị ghim ở **bốn** chỗ (checklist §5 · ô `Đầu ra kiểm chứng được` của T-04 · bảng `cl-T-04` · ô biên nhận của [T-33](task.md) còn mở), nên checklist **không có cửa** nhận vế XONG mới — kể cả vế học từ một sự cố thật mà §6.7 luật 2 đòi | rà quality/05-checklist.md · 2026-08-24 | 🔴 MỞ | ⚠️ chưa có task | `project_preparation/prompt-fullstack.md` `quality/00-guideline-chat-luong.md` `quality/05-checklist.md` `task.md` |
| [F-35](#f-35) | Bảy ô `- [ ]` ở §1 **không phải bảy phép xác nhận**: 5/7 dòng gộp nhiều thứ vào một dấu tick (vế 1 gộp `build · lint · unit test · typecheck`), tổng ra **14–16 tuỳ cách đếm** — và `grep -c` đang đo **số dòng**, không đo số thứ đã xác nhận | rà quality/05-checklist.md · 2026-08-24 | 🔴 MỞ | ⚠️ chưa có task | `quality/05-checklist.md` |
| [F-36](#f-36) | §6.9 *(ba thứ không bao giờ thoả hiệp)* chỉ được **trỏ** ở 3 chỗ, **không** cổng nào buộc xác nhận nó trước commit: §3 checklist (*cái gì không tính là XONG*) nhắc `0` lần, và `0` ô `Đạt khi` nào của bảng ba cổng [guideline §6](quality/00-guideline-chat-luong.md) nhắc tới nó | rà quality/05-checklist.md · 2026-08-24 | 🔴 MỞ | ⚠️ chưa có task | `CLAUDE.md` `project_preparation/prompt-fullstack.md` `quality/00-guideline-chat-luong.md` `quality/05-checklist.md` |
| [F-37](#f-37) | Lời hứa ở checklist §2 — *"vế nào còn ⚠️ thì lane sinh ra nó gỡ ⚠️ ngay trong task mở lane"* — **không nằm trong sổ của ai**: 7/8 dòng bảng §2 đang ⚠️, mà `0` dòng task của T-07 T-08 T-09 nhắc tới `quality/05-checklist.md` | rà quality/05-checklist.md · 2026-08-24 | 🔴 MỞ | ⚠️ chưa có task | `CLAUDE.md` `quality/05-checklist.md` `task.md` |
| [F-38](#f-38) | Vòng lặp con trỏ [rule §5.1](.claude/rules/quan-ly-du-an.md) **mù với neo `#f-xx`**: lớp `[^)#]*` cắt bỏ mọi thứ sau `#`, nên `](../finding.md#f-67)` chỉ bị kiểm là *file có tồn tại không* → xanh — và đang có một neo hụt sống ở `prompt-fullstack.md` dòng 13 (`grep -c '^### F-67' finding.md` ra `0`) | rà quality/05-checklist.md · 2026-08-24 | 🔴 MỞ | ⚠️ chưa có task | `.claude/rules/quan-ly-du-an.md` `finding.md` `project_preparation/prompt-fullstack.md` `quality/05-checklist.md` `task.md` |
| [F-39](#f-39) | Luật báo cáo chỉ đòi con trỏ xuống tới **mục** — ô `Sửa ở đâu` ở [.claude/rules/bao-cao-thay-doi.md §1](.claude/rules/bao-cao-thay-doi.md) đặc tả `§1.1 bảng đặc tả ô`, `dòng T-05` — mà **không** đòi **đường dẫn** đi kèm, nên `§6` trần lọt hợp lệ vào mọi báo cáo; repo đang có **5** file `.md` cùng có một mục `§6`, owner đọc `§6` không biết là §6 của file nào | owner nêu · 2026-08-25 | ✅ ĐÓNG 2026-08-25 | ⚠️ đóng ngay trong phiên owner nêu — không đẻ task | `.claude/rules/bao-cao-thay-doi.md` `.claude/rules/quan-ly-du-an.md` `CLAUDE.md` `quality/00-guideline-chat-luong.md` |
| [F-40](#f-40) | Hệ đánh số tên file `quality/` va nhau: [guideline §7](quality/00-guideline-chat-luong.md) đặt tầng pha ở `quality/0n-pha-<n>-<tên>.md` cho **6 pha** ⇒ pha 4 đòi `05-`, pha 5 đòi `06-`; nhưng `05-` đã là `05-checklist.md` (T-04 đã xong) và `06-` đã bị [T-33](task.md) đặt trước cho `06-rang-buoc-kien-truc.md` — hai chỗ va, cả hai đều đã ghim vào sổ | T-34 · 2026-08-25 | 🔴 MỞ | ⚠️ chưa có task | `project_preparation/prompt-fullstack.md` `quality/00-guideline-chat-luong.md` `quality/05-checklist.md` `task.md` |
| [F-41](#f-41) | `NHA` trong [Makefile](Makefile) là **danh sách gõ tay 10 file**, không cơ chế nào bắt một nhà mới bị bỏ ngoài: `quality/05-checklist.md` ra đời ở T-04 và **chưa từng** vào `NHA`, nên bốn đích của `make check` không hề soi nó — cổng xanh trong khi hai nhà đứng ngoài cổng | T-34 · 2026-08-25 | 🔴 MỞ | ⚠️ chưa có task | `Makefile` `quality/05-checklist.md` `task.md` |
| [F-42](#f-42) | Nhịp *"chạy sáu probe N1–N6 rồi dán phiếu, trước biên nhận"* mà [quality/01-chat-luong-noi-dung.md §5](quality/01-chat-luong-noi-dung.md) đặt ra **không có nhà**: §7 file đó nhượng nhịp cho [.claude/rules/quan-ly-du-an.md](.claude/rules/quan-ly-du-an.md) mục 4 rồi tự cấm mở nhịp thứ hai, nhưng mục 4 có **9** hàng và **không hàng nào** là sáu probe — thủ tục chấm nội dung tồn tại như văn bản, không tồn tại như nhịp | owner hỏi hiện trạng · 2026-08-25 | ✅ ĐÓNG 2026-08-25 | ⚠️ đóng ngay trong phiên phát hiện — không đẻ task | `.claude/rules/quan-ly-du-an.md` `CLAUDE.md` `quality/01-chat-luong-noi-dung.md` |
| [F-43](#f-43) | `paths:` của [.claude/rules/quan-ly-du-an.md](.claude/rules/quan-ly-du-an.md) liệt kê **7** mẫu và **không mẫu nào khớp thư mục code**; [rule mục 6](.claude/rules/quan-ly-du-an.md) — 5 vế mở một lane — chỉ đòi `paths:` cho `.claude/rules/lane-<tên>.md`, không vế nào đòi nới `paths:` của rule quản lý dự án ⇒ từ lúc [T-07](task.md)/[T-08](task.md)/[T-09](task.md) mở lane DB/BE/FE, phiên viết code không nạp rule này, và hàng nhịp vừa thêm ở [F-42](#f-42) tắt đúng lúc đầu ra là code | owner hỏi hiện trạng · 2026-08-25 | ✅ ĐÓNG 2026-08-26 | ⚠️ T-36 đã huỷ khỏi sổ 2026-08-26 — không đẻ task thay thế | `.claude/rules/chat-luong-finding.md` `.claude/rules/quan-ly-du-an.md` `CLAUDE.md` `finding.md` `quality/01-chat-luong-noi-dung.md` `task.md` |
| [F-44](#f-44) | **Định nghĩa XONG thiếu một vế.** [.claude/rules/quan-ly-du-an.md](.claude/rules/quan-ly-du-an.md) mục 3 vế 4 khai *"**Ba** thứ của `CLAUDE.md` §4 đủ"* rồi liệt kê 3 — biên nhận · commit · `finding.md` — nhưng [CLAUDE.md §4](CLAUDE.md) khai *"cần đủ **bốn** thứ"*, thứ (4) là **bảng thay đổi**; phiên theo đúng rule mục 3 đánh ✅ hợp lệ mà không dán bảng, đúng thứ CLAUDE.md §4 gọi là *phiên chưa xong* | T-36 · 2026-08-25 | ✅ ĐÓNG 2026-08-26 | ⚠️ đóng ngoài sổ, cùng phiên nâng trần | `.claude/rules/bao-cao-thay-doi.md` `.claude/rules/quan-ly-du-an.md` `CLAUDE.md` `finding.md` |
...
**Ca sống 2026-08-25 — lệnh 3 đỏ ngay lần chạy đầu.** Ô `Đầu ra` của `T-03` trong [task.md](task.md)
...
**Ca sống thứ năm — 2026-08-26: không mất hunk, mất *bản khai*.** Phiên NON-CODE này thấy cây bẩn **3 file** ở
bước 4 ([CLAUDE.md §3](CLAUDE.md)), khai đủ ba file vào message rồi `git add task.md finding.md CLAUDE.md`.
Trong lúc nó đọc rule, phiên khác commit `7c3f7f0` đúng `task.md` + `finding.md` với **cùng nội dung** — nên
`git add` stage được đúng **một** file, và commit ra đời chứa `CLAUDE.md` trong khi message khai ba.
**Không hunk nào bị nuốt**: đây là biến thể rẻ nhất của mục này, và chính vì rẻ nên nó dễ trôi nhất — cây sạch,
biên nhận xanh, chỉ `git log` sai, mà `git log` là nhà duy nhất của *ai sửa file nào* ([CLAUDE.md §2](CLAUDE.md)).
Bắt được chỉ vì phiên chạy `git show --stat` **ngay sau** commit; sửa bằng `git commit --amend` (`a89a0be` →
`0d5dc42`, chưa ai ghim nên amend không giết con trỏ nào — luật ghim sha ở [F-46](#f-46)).
Thêm vế cho `Cách sửa đề xuất`, rẻ hơn mọi cơ chế khoá: **mỗi commit xong chạy `git show --stat <sha>` và đối
...
(*"Chạy hai lệnh, đúng thứ tự"*), nhưng **không file nào trong repo gọi nó nữa**. Caller duy nhất là
[CLAUDE.md §3](CLAUDE.md) bước 2, và bước 2 nay khai *prompt là nguồn việc duy nhất* (owner chốt 2026-08-26,
bỏ **hẳn** vế dò sổ chứ không hạ xuống fallback). Cùng lúc
[project_preparation/huong-dan-viet-task-md.md §5.12](project_preparation/huong-dan-viet-task-md.md) — nhà của
*luật viết sổ task* theo [CLAUDE.md §2](CLAUDE.md) — vẫn bắt mục đó *"phải là **lệnh chạy được** dò theo đúng
các tầng này"*. Hai nhà đang khai ngược nhau, cả hai đều có hiệu lực.

**Vì sao nó không tự mất đi.** Chạy hết [task.md](task.md) y như nó viết: [T-30](task.md) siết khuôn message
commit, [T-32](task.md) thêm vế `Đóng khi.` cho sổ lỗi, [T-33](task.md) tạo nhà cho ràng buộc kiến trúc,
[T-35](task.md) dựng hook `Stop` — không dòng nào chạm mục *Task tiếp theo* hay §5.12. Ngược lại, mỗi phiên sau
đọc `task.md` từ trên xuống lại gặp một bộ dò trông còn hiệu lực và chạy nó, tức cái sai **được dùng** chứ không
nằm im. Còn ⇒ finding.

**Lệnh tái hiện.** Ba vế, chạy từ gốc repo:

```bash
grep -c 'Task tiếp theo' CLAUDE.md                                   # ra 0  <- caller đã mất
sed -n '/^## Task tiếp theo/,/^---/p' task.md | grep -c '^grep -n'   # ra 2  <- bộ dò vẫn còn, vẫn 2 lệnh
grep -c 'lệnh chạy được' project_preparation/huong-dan-viet-task-md.md   # ra >= 1 <- nhà kia vẫn bắt buộc
```

Vế 1 ra `0` **và** vế 2 ra `2` là đủ dựng mâu thuẫn: một mục còn nguyên, không ai gọi.

**Đóng khi.** Ba vế trên hết mâu thuẫn — hoặc vế 1 ra `>= 1` (có caller mới, hợp lệ), hoặc vế 2 ra `0` (bộ dò
đã rút khỏi `task.md`) **và** §5.12 sửa theo. **Đỏ khi** vế 1 ra `0` mà vế 2 vẫn ra `2`: đúng trạng thái hôm nay.

**Cách sửa đề xuất.** Chốt **hướng (a) — giữ bộ dò, đổi vai thành *tra cứu theo yêu cầu*, không phải *nguồn việc***:
sửa lời mở của mục thành *"người dùng hỏi còn việc gì thì chạy hai lệnh này"*, và sửa §5.12 của
[huong-dan-viet-task-md.md](project_preparation/huong-dan-viet-task-md.md) cho khớp vai mới.
Hướng (b) — **xoá hẳn** mục khỏi `task.md` — bị loại: `task.md` vẫn là nhà của *làm gì tiếp, theo thứ tự nào*
([CLAUDE.md §2](CLAUDE.md)), và xoá bộ dò thì câu hỏi đó chỉ còn trả lời được bằng cách đọc cả sổ — đắt hơn hẳn
việc sửa một dòng lời mở. Cả hai hướng chạm **2 file**, một lane NON-CODE, vừa [CLAUDE.md §6](CLAUDE.md).

`Ngoài: Makefile — mục tiêu không ai gọi (dead target), lint bắt bằng đồ thị phụ thuộc` · `Ta:` bắt bằng
`grep -c` một chiều, không dựng đồ thị · `Vì:` repo này có đúng một caller cho mỗi mục, đồ thị là công cụ của
hệ hàng nghìn đích, không phải của một sổ hai chục dòng.

**Bẫy khi sửa.** Đừng nhân tiện đóng luôn [F-18](#f-18): mệnh đề của F-18 khai *"bước 2 của §3 dò đúng vào nó"*
...
[.claude/rules/quan-ly-du-an.md](.claude/rules/quan-ly-du-an.md) khai điều kiện kiểm là ``wc -l` ≤ 120` và
hậu quả là *"gộp hoặc thay luật cũ, **không nới trần**"*. Cả hai vế đã hết đúng ở commit `78a8901`
(2026-08-26): [CLAUDE.md §8](CLAUDE.md) nay ghi trần **140 dòng**, và ghi rõ *owner nới ngày 2026-08-26,
có hạn*. Theo [CLAUDE.md §2](CLAUDE.md), nhà thật thắng ⇒ hàng rule này là **bug phải sửa**, không phải
một cách đọc khác. Gốc sâu hơn: rule **chép số** của một nhà khác thay vì trỏ tới nó.

**Vì sao nó không tự mất đi.** Chạy hết [task.md](task.md) y như nó viết: [T-30](task.md) sửa
[CLAUDE.md §4](CLAUDE.md) và §5.2 của rule, [T-32](task.md) sửa sổ lỗi, [T-33](task.md) sửa
[CLAUDE.md §2](CLAUDE.md), [T-35](task.md) sửa `.claude/settings.json` — không dòng nào chạm mục 4.
Bằng chứng mạnh hơn: chính commit `78a8901` nới trần **có** sửa mục 3 của rule này mà **không** sửa mục 4,
tức lệch này đã sống sót đúng cái phiên sinh ra nó. Còn ⇒ finding.

**Lệnh tái hiện.** Hai vế, chạy từ gốc repo:

```bash
grep -c '≤ 120' .claude/rules/quan-ly-du-an.md   # ra 1  <- rule còn ghim số cũ
grep -c '140 dòng' CLAUDE.md                     # ra 1  <- nhà thật đã là 140
```

**Đóng khi.** Vế 1 ra `0` — rule hết ghim số. **Đỏ khi** vế 1 ra `>= 1` trong lúc vế 2 vẫn ra `1`: đúng
trạng thái hôm nay, hai nhà khai hai con số.

**Cách sửa đề xuất.** Chốt **bỏ con số khỏi rule, thay bằng con trỏ**: ô điều kiện của hàng đó viết
*"ba cổng ở [CLAUDE.md §8](CLAUDE.md)"*, ô hậu quả bỏ vế *"không nới trần"* (nới trần là quyền owner theo
[CLAUDE.md §7](CLAUDE.md), rule lane không cấm được). Hướng *sửa `120` thành `140`* bị **loại**: nó xanh
đúng tới lần nới trần kế, rồi lệch lại — và đó chính là cách lệch hôm nay ra đời. Một file, lane NON-CODE.
...
**Mệnh đề sai.** Cổng **(a)** ở [.claude/rules/chat-luong-finding.md §3](.claude/rules/chat-luong-finding.md) — cổng duy nhất bắt *mục 🔴 thiếu vế bắt buộc* — dò bốn tiêu đề vế bằng **chuỗi cứng có dấu chấm cuối**. Ba mục viết **đúng tinh thần** nhưng khác ký tự đều bị nó gọi là thiếu vế, nên 3/4 dòng output hôm nay là báo nhầm:

| Mã | Nó in ra | Thực tế trong file | Vì sao lệnh trượt |
|---|---|---|---|
| `F-06` | `THIẾU VẾ: F-06 Lệnh-tái-hiện` | có `**Lệnh tái hiện**` | mẫu đòi `\*\*Lệnh tái hiện\.\*\*` — thiếu đúng **một dấu chấm** |
| `F-07` | `THIẾU VẾ: F-07 Cách-sửa` | có `**Cách sửa đề xuất — kiến trúc, agent không tự làm…**` | mẫu đòi `\*\*` đóng **ngay sau** cụm; hậu tố làm trượt |
| `F-12` | `THIẾU VẾ: F-12 Cách-sửa` | như `F-07`, cộng `**Chưa đóng được vì:**` | sổ dùng **hai** cách viết cho cùng một vế, mẫu chỉ nhận `**Chưa đề xuất được vì:**` |
| `F-13` | `THIẾU VẾ: F-13 Vì-sao-không-tự-mất` | **thiếu thật** | — |

**Vì sao nó nguy hiểm hơn nó trông.** Cổng sai **theo chiều báo thừa** chết nhanh hơn cổng bỏ sót: người đọc thấy 4 dòng, kiểm 2 dòng đầu thấy sai, rồi bỏ qua cả output. Ca thật duy nhất — `F-13` — nằm ở dòng thứ tư và đã sống sót đúng như vậy. Cổng này cũng **không** nằm trong đích nào của [Makefile](Makefile), nên không lần chạy nào của `make check` in nó ra.

**Vì sao nó không tự mất đi.** Chạy hết [task.md](task.md) y như nó viết: không dòng task nào chạm cổng (a) hay khuôn tiêu đề vế. Mẫu chuỗi cứng còn nguyên, và mỗi mục finding mới viết hơi khác một ký tự lại đẻ thêm một báo nhầm.

**Lệnh tái hiện.**

```bash
# in ra 4 dòng; 3 dòng đầu là báo nhầm — đối chiếu ngay bằng lệnh dưới
for f in $(grep '^| \[F-[0-9]*\].*🔴 MỞ' finding.md | sed 's/^| \[\(F-[0-9]*\)\].*/\1/'); do
  s=$(sed -n "/^### $f\$/,/^### F-/p" finding.md); m=""
  echo "$s" | grep -q '^\*\*Lệnh tái hiện\.\*\*' || m="$m Lệnh-tái-hiện"
  echo "$s" | grep -qE '^\*\*(Cách sửa đề xuất\.|Chưa đề xuất được vì:)\*\*' || m="$m Cách-sửa"
  [ -n "$m" ] && echo "THIẾU VẾ: $f$m"; done
# chứng minh 3 dòng đầu là nhầm: ba mục ấy CÓ vế, chỉ khác ký tự
for f in F-06 F-07 F-12; do sed -n "/^### $f\$/,/^### F-/p" finding.md \
  | grep -oE '^\*\*(Lệnh tái hiện|Cách sửa đề xuất|Chưa [^*]*)[^*]*\*\*'; done
```

**Đỏ khi:** lệnh thứ nhất in ra mã nào mà lệnh thứ hai chứng minh mục đó **có** vế. Hết đỏ khi hai lệnh không còn mã chung.
...
**Bẫy khi sửa.** Đừng sửa theo chiều ngược lại (bắt ba mục viết đúng chuỗi cứng): [.claude/rules/chat-luong-finding.md §1](.claude/rules/chat-luong-finding.md) đã nói tiêu đề *"phải viết đúng từng ký tự"*, nhưng chính sổ đã trôi khỏi luật đó ở 3/51 mục, và hậu tố của `F-07` `F-12` (*"— kiến trúc, agent không tự làm"*) đang **mang thông tin**, cắt đi là mất.
